...
class CheckoutService
{
    ...
    public function complete(User $user, array $pending, string $paymentMethod): void { $payload = ['payment_method' => $paymentMethod]; if ($pending['type'] === 'single') $this->transactionService->store($payload + ['product_id' => $pending['product_id'], 'quantity' => $pending['quantity']]); else $this->transactionService->checkoutCart($payload); }
}
